...
function accordion_group_shortcode($atts, $content = null){
	...
	return '<div id="accordion">'.$content.'</div>';
}
...
function tabs_shortcode($atts, $content = null){
	...
	$content = do_shortcode($content);
	$tabs_parts = explode('(flowuniqueid-4a1l40346xfear7)', $content);
	foreach($tabs_parts as $tabs_part){
		$tabs_part = explode('(flowuniqueid-4a1l40346xfear6)', $tabs_part);
...
function tab_shortcode($atts, $content = null){
	...
	if($atts['open'] == '1'){
		$tab_active_class = ' ui-tabs-selected ui-state-active'; 
		$tab_open_class = '';
	}else{ 
		/* $tab_active_class = ''; 
		$tab_open_class = 'ui-tabs-hide'; */
	}
	
	$tabs_code = '<li class="'.$tab_active_class.'"><a href="#'.$uniqid.'">'.$atts['title'].'</a></li>(flowuniqueid-4a1l40346xfear6)<div id="'.$uniqid.'"><p>'.$content.'</p></div>(flowuniqueid-4a1l40346xfear7)';
