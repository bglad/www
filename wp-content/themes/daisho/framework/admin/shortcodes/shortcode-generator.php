...
<table class="form-table">
	<tr>
		<th style="width:20%;">
			<label for="<?php _e('shortcode_editor', 'flowthemes') ?>"><?php _e('Shortcode Generator', 'flowthemes'); ?></label>
		</th>
		<td>
			...
			<?php foreach($shortcode_boxes as $shortcode_box_key => $shortcode_box_value){
				...
				foreach($shortcode_box_value['fields'] as $shortcode_field_key => $shortcode_field_value){
					if($shortcode_field_value[0] == 'input'){
						if($shortcode_field_value[3] == 'required'){ $field_required = "<span class=\"flow-shortcode-label-required\">*</span>"; $required_class = 'flow-shortcode-field-required'; }else{ unset($field_required); unset($required_class); }
