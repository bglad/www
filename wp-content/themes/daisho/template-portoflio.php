...
<nav id="main-nav-compact" style="display: none; opacity: 0;">
	<div class="clearfix main-nav-compact-inner">
		...
		<?php wp_nav_menu( array( 'sort_column' => 'menu_order', 'theme_location'=>'main_menu', 'menu_class'=>'menu-compact', 'menu_id'=>'menu-compact' ) ); ?>
	</div>
</nav>
<div class="project-navigation" style="height: 80px;">
			...
			<?php
				foreach ($tax_terms as $tax_term) {
					...
					if( (is_array($pf_categorynotin)) && in_array(sanitize_title($tax_term->slug), $pf_categorynotin) ){
						...
						echo '<li>' . '<a href="#filter" data-option-value=".' . sanitize_title($tax_term->name) . '">' . $tax_term->name  . '</a></li>'; /* Use slug instead for filter version? */
					}
				}
			?>
		</ul>
	...
	if( have_posts() ) : 
		while ($wp_query->have_posts()) : $wp_query->the_post();
			...
			if ($attachments or $thumbnail_hover_color) {
				...
				$post_cats = array();
				for($h=0;$h<count($post_cat);$h++){
					$post_cats[] = $post_cat[$h]->name; /* Use slug instead for filter version? */
				}
				$cats_pf_this = implode(", ", $post_cats); /* Thumbnail print version */
				...
				$cats_pf_this_class = strtolower(implode(" ", str_replace(" ", "-", $post_cats))); /* Category filter version */
				$cprojvalid = false;
